import { z } from "zod";
import { clientIdSchema, credentialsSchema, itemsSchema, revisionSchema, timestampSchema } from "./protocol";
...
export const INITIAL_STATE: State = stateSchema.parse({
  config: null, status: { phase: "unpaired", lastSyncAt: null, error: null },
  items: [], revision: 0, sequence: 0, clock: 0, pending: null,
});

export async function loadState(): Promise<State> {
  const stored = await chrome.storage.local.get("state");
  return stored["state"] === undefined ? INITIAL_STATE : stateSchema.parse(stored["state"]);
}
...
export const viewSchema = z.strictObject({
  label: z.string(), paired: z.boolean(), status: statusSchema,
  revision: revisionSchema, count: z.number().int().nonnegative(),
}).readonly();

export function stateView(state: State): z.infer<typeof viewSchema> {
  return {
    label: state.config?.label ?? "", paired: state.config !== null, status: state.status,
    revision: state.revision, count: state.items.filter((item) => item.deletedAt === null).length,
  };
}
...
export const messageSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({
    type: z.literal("configure"), label: z.string().trim().min(1).max(64),
